...
public class LocationDeletionValidator {
	...
	public void validateDeletionPossible(Long locationId) {
		...
		stockRepository.findAllByWarehouseId(locationId).forEach(stock -> relatedWareList.add(stock.getWareId()));
		...
		logisticTaskRepository.findAllByFromLocationIdOrToLocationId(locationId, locationId).forEach(task -> relatedTaskList.add(task.getId()));
		...
		logisticTemplateRepository.findAllByFromLocationIdOrToLocationId(locationId, locationId).forEach(template -> relatedTemplateList.add(template.getId()));
