...
@SpringBootTest
@ActiveProfiles("test")
@DisplayName("LogisticTask-Stock Redis Stream 통합 테스트")
class LogisticTaskStockIntegrationTest {
    ...
    void setUp() {
        ...
        // 초기 재고 설정 (창고A에 노트북 50개)
        stockCtrlService.create(StockDTO.CreateReq.builder().warehouseId(warehouseA.getId()).wareId(laptop.getId()).quantity(30).build());

        // 인증 설정
        SecurityContextHolder.getContext().setAuthentication(
                new UsernamePasswordAuthenticationToken(worker, null, null));

        log.info("테스트 데이터 준비 완료 - 작업자: {}, 물품: {}, 창고A: {}, 창고B: {}",
                worker.getId(), laptop.getId(), warehouseA.getId(), warehouseB.getId());
    }
