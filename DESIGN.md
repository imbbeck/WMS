...
-   **비즈니스 규칙**:
    ...
    -   STARTED 로 상태변경 시 MovementStarted 이벤트 발행
        -   이벤트 구독자가 재고 변경을 수행 (비동기 처리)
    -   COMPLETED 로 상태변경 시 MovementCompleted 이벤트 발행
        -   이벤트 구독자가 재고 변경을 수행 (비동기 처리)
...
## 물류이동 계획 생성 시 재고/카파 정합성 검증(수정시에는 어떻게 하지?..)

-   시간대(계획된 물류이동의 예정 시작시간, 완료시간)별 예측 재고 현황을 캐싱. 새로운 물류이동을 배정할때 캐싱된 재고, 그를 이용한 카파와 비교해 validation 수행

    -   물류이동 계획 생성시에는 무조건 선형으로 물류이동 배정해야됨. 실수했으면 지우고 첨부터 다시..
    -   물류이동 계획은 수정불가

-   향후: 그래프 기반 작업 흐름 (DAG) + 영향도 추적하여 수정 시 영향받는 하위 작업만 재검증

---

## 작업계획 무오성 검사 시뮬레이션 설계

### 목적

작업계획 생성 후 하루 단위의 작업계획들 실제 실행 전 미리 오류(재고 부족, 창고 용량 초과 등)를 사전 탐지하여 계획의 무오성을 검증하는 시스템.
...
여러 작업자가 동시에 같은 재고나 물류이동 건에 대해 `시작` 또는 `완료 처리`를 시도할 경우, DB 업데이트 충돌이 발생할 수 있음.
