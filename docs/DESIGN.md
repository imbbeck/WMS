...
-   재고 등 캐싱을 활용한 성능 개선
-   MQ(redis stream) 기반 비동기 처리로 동시성 문제 해결
...
-   로깅 및 장애 알림 MQ 적용
-   최선의 물류이동 계획(유휴시간 최소화 등)를 제시하는 시스템
...
-   물품관리 페이지
    - 물품 별 재고 현황 페이지 팟업으로
-   재고관리 페이지. 장소 별/물품 별 재고 현황 페이지에서 제어할 수 있게 한다면 필요없을 수도 있음
-   물류작업템플릿 crud 페이지
-   물류작업 계획 페이지 - 작업자들의 타임테이블이 있고 물류작업템플릿 카드박스에서 카드를 드래그앤드랍으로 배치함으로써 물류작업 c. ud는 배치된 카드 수정으로.
-   전체 물류이동 상황 현황. 물류작업 계획 페이지와 유사하나, 전체 물류이동 상황을 한눈에 볼 수 있는 페이지. 통합여부 검토 필요
-   작업자별 물류작업 계획 현황 - 배정받은 물류작업 계획 조회 및 물류작업상태 변경(완료 처리 등) 제어패널의 역할도 하는 페이지

TODO: 모든 현황 페이지 sse로 실시간 업데이트. 현재 는 물류작업 페이지 내 시작처리/완료 처리 시에만 재고 변동 처리 알림만 발행하고 있음.
...
-   **비즈니스 규칙**:
    ...
    -   물류이동 작업은 자기자신 상태변화에 대해서만 리턴, 재고변화는 redis stream 기반으로 하고, status를 따로 sse로 발행. 큐 기반이기 때문에 기존 낙관적 락 + 지수적 백오프 재시도 전략은 사용안하지만 관련 코드는 남겨둠
    -   여기서만 redis stream을 사용하고, 기존 spring event로 처리되는 다른 이벤트는 그대로 유지
    -   비동기처리의 근본적인 문제. 재고엔티티에서 재고없을때 물류작업시작 불가/창고여유없을때 물류작업완료 불가 처리를 위해 예외발생시키는데 이는 물류작업상태변경api에서도 알아야될 사항이지만 전파가 안됨. -> 별도의 재고 변경 처리 상태에 대한 sse 발행으로 ui/ux에서 처리.
...
1. **작업별 예상 시작/완료시간 계산**
    - 이전작업 도착장소 ~ 다음작업 출발장소 간 이동소요시간 고려
2. **시간 순으로 재고 변화 예상**
...
### 대응 전략: MQ(redis steam)을 이용하여 비동기적 순차처리로 1차대응, 낙관적 락 + 지수적 백오프 + 사용자 재시도 유도(재고변동 처리 상황을 sse로 발행하여 사용자에게 알림)으로 2차대응
...
> MQ 적용의 사이드이펙트로 사용자가 즉각적 피드백을 받지 못함. 처리 상태를 지속적으로 확인할 수 있도록 sse로 상태 변경 알림을 발행하여 사용자에게 실시간으로 처리 상황을 전달함.
...
- 재고 상황 DB, 캐시 일치 여부 검사. 검사 후 재고 상황이 일치하지 않는 경우 로깅하고 재고 상태 캐싱 갱신
- 전일 스냅샷 + 오늘의 총 물류작업의 결과와 재고 상황 일치 여부 검사. 검사만 수행하고, 재고 상황이 일치하지 않는 경우 로깅.(관리자의 직접적 재고 변동 처리가 존재했었을 수 있기 때문에 검사만 수행)

---

## TODO: 물류작업기록 매일 자정 결산 배치 프로세스

재고 배치 프로세스 이후 실행되는 Spring Batch 기반 스냅샷 생성 프로세스. (재고기록 배치, 물류작업기록 배치 순서는 고민 필요)
- 오늘의 작업결과를 logisticTaskHistory로 저장 후 task에서 삭제(앞으로 실행될 물류작업에 대한 테이블이므로).
    - PENDING, INITIATE_DELAYED 상태의 작업들은 EXPIRED 상태로 저장.
    - INITIATED, COMPLETE_DELAYED, FAILED 상태의 작업들은 FAILED 상태로 저장.
    - COMPLETED, CANCELLED 상태의 작업들은 자기 상태그대로 저장.
